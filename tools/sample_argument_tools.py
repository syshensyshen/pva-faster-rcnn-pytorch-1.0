...
def lightexchange(img):
    ...
    width  = img.shape[1] >> 1
    center_x = (height) #+ np.random.randint(-height>>2, height>>2)
    center_y = (width) #+ np.random.randint(-width>>2, width>>2)  
    center_1 = (center_x, center_y)
    scale = random.uniform(1.0, 1.1)
    power = random.uniform(1.0, 1.2)
    center_x = (height) + np.random.randint(-height>>1, height>>1)
    center_y = (width) + np.random.randint(-width>>1, width>>1)  
    center_2 = (center_x, center_y)
    #if scale < 1.0:
    #    center_1 = center_2 = (width, height)
