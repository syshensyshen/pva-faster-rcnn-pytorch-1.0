...
class faster_rcnn(nn.Module):
    ...
    def __init__(self, cfg, classes, pretrained=False, align=False):
        ...
        self.Regression = nn.Sequential(OrderedDict([
            ('fc6', nn.Linear(self.ROIDim * cfg.POOLING_SIZE *
                              cfg.POOLING_SIZE, self.regressionDim)),
    ...
    def forward(self, im_data, im_info, gt_boxes):
        ...
        self.rois = self.proposallayer(
            (RPN_cls_score.data, RPN_bbox_pred.data, im_info, cfg_key))
        ...
        if self.training:
            ...
            self.bbox_targets = Variable(
                self.bbox_targets.view(-1, self.bbox_targets.size(2)))
        ...
        # Reshape into (batchsize, all)
        roi_feat = self.roi_feat.view(self.roi_feat.size(0), -1)
