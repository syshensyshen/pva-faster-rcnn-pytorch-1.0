...
def _smooth_l1_loss_by_zcc(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
...
def _balance_smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    alpha = 0.5
    gamma = 1.5
    balance = np.exp(gamma / alpha) - 1
    sigma_2 = sigma ** 2
    bbox_diff = torch.abs(bbox_pred - bbox_targets)
    bbox_diff = bbox_inside_weights * bbox_diff
    smoothL1_sign = (bbox_diff < 1. / sigma_2).detach().float()
    in_loss_box = (alpha / balance * torch.log(balance * bbox_diff + 1.0) - alpha * bbox_diff) * smoothL1_sign \
        + (gamma * bbox_diff + 0.5) * (1.0 - smoothL1_sign)
...
def kl_bbox_regression_loss(bbox_pred, bbox_std_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, stage=1.0, dim=[1]):
    ...
    loss_bbox = torch.where(cond, calcLreg(
        bbox_diff, bbox_std_pred, True), calcLreg(bbox_diff, bbox_std_pred, False))
