...
class XML_VOCDataset:
    ...
    def __init__(self, cfg, img_path, xml_path, class_list, transform=None):
        ...
        # added by Henson
        # self.scales = [s for s in range(
        #     cfg.TRAIN.scales[0], cfg.TRAIN.scales[1], cfg.SCALE_MULTIPLE_OF)]
        # self.scale_multiple_of = cfg.SCALE_MULTIPLE_OF
    ...
    def __getitem__(self, index):
        ...
        # sample = {'img': img, 'annot': annot, 'filename': img_path, 'scales': self.scales,
        #           'scale_multiple_of': self.scale_multiple_of, 'max_size': self.max_size}
    ...
    def _get_annotation(self, annotation_file):
        ...
        for object in objects:
            ...
            for key in self.cfg.rename_class_list.keys():
                if class_name in self.cfg.rename_class_list[key]:
                    class_name = key
                    break
            ...
            h = y2 - y1
            w = x2 - x1
            # fliter too small boundingboxes
            # those boxes not positive sample
            if w < 5 and h < 5:
                continue
            # if h < 24 and w < 24:
            #     if class_name in ['SL', 'HH']:
            #         class_name += '1'
            annotation = np.zeros((1, 5))
            annotation[0, 0] = x1
            annotation[0, 1] = y1
...
def collater(data):
    ...
    shapes = [shape for shape in range(1024, 1280, 32)]
