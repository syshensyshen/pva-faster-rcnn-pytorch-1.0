...
class XML_VOCDataset:
    ...
    def __init__(self, img_path, xml_path, class_list, transform=None):
        """
...
def collater(data):
    ...
    for i in range(batch_size):
        ...
        im_info[i, :] = torch.from_numpy(np.array([img.size(0), img.size(1), scales[i], scales[i]], dtype=float))
...
class ResizerMultiScale(object):
    def __call__(self, sample, min_side=416, max_side=864):
