...
class _ProposalTargetLayer(nn.Module):
    ...
    def forward(self, all_rois, gt_boxes):
        ...
        new_all_rois = torch.zeros(
            (all_rois.size(0), all_rois.size(1)+gt_boxes.size(1), all_rois.size(2)))
        ...
        new_all_rois[:, all_rois.size(1):all_rois.size(
            1) + new_gt_boxes.size(1), :] = new_gt_boxes
    ...
    def _get_bbox_regression_labels_pytorch(self, bbox_target_data,
        ...
        for b in range(batch_size):
            ...
            for i in range(inds.numel()):
                ...
                bbox_inside_weights[b, ind,
                                    start:end] = self.BBOX_INSIDE_WEIGHTS
